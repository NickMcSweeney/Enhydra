...
module.exports = {
  saveFile: async (data, db, mongo) => {
    try {
      ...
      const ret = await gfs.fromFile({ filename }, source, async (err, file) => {
        const data = await gfs.toFile({ _id: file._id }, dir + filename, err => {
          let fileContents = fs.readFileSync(dir + filename).toString();
          console.log("wrote file %s to %s: %s", file._id, filename, fileContents);
        });
        return file;
      });
      return ret.id;
  ...
  writeFileToDb: async (data, db, mongo) => {
    try {
      ...
      const ret = await gfs.fromFile({ filename }, source, async (err, file) => file);
      return ret.id;
    } catch (e) {
      console.error(e);
    }
  ...
  removeFile: async (filename, db, mongo) => {
    ...
    try {
      ...
      if (found) {
        ...
        if (ret) await fs.unlink(dir + filename);
